...
const translations = {
    ...
    ru: {
        ...
        tasks_label: "Задачи", importances_label: "Важиости", time_label: "Время", schedule_label: "Расписание",
...
function setupAds() {
    ...
    bigAd.innerHTML = adConfig.big.text[currentLang] ? `
        <img src="${adConfig.big.imageSrc}" alt="Big Ad" class="w-12 h-12 rounded object-cover">
    ...
    bigAd.className = adConfig.big.text[currentLang] ? 
        'w-full max-w-md mt-4 bg-yellow-100 rounded-lg shadow p-4 flex items-center space-x-4 h-24 slide-in cursor-pointer' : 
        'w-full max-w-md mt-4 bg-yellow-100 rounded-lg shadow p-4 flex items-center space-x-4 h-24 slide-in ad-empty';
    bigAd.onclick = adConfig.big.url ? () => window.open(adConfig.big.url, '_blank') : null;

    const smallAd = document.getElementById('small-ad');
    smallAd.innerHTML = adConfig.small.text[currentLang] ? `
        <img src="${adConfig.small.imageSrc}" alt="Small Ad" class="w-8 h-8 rounded object-cover">
    ...
    smallAd.className = adConfig.small.text[currentLang] ? 
        'w-full max-w-md mt-4 mb-4 bg-yellow-100 rounded-lg shadow p-4 flex items-center space-x-4 h-16 slide-in cursor-pointer' : 
        'w-full max-w-md mt-4 mb-4 bg-yellow-100 rounded-lg shadow p-4 flex items-center space-x-4 h-16 slide-in ad-empty';
