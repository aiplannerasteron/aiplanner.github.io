const adConfig = {
    small: {
        ...
        imageSrc: "https://postimg.cc/kVHqDhzr"
    },
    big: {
        text: {
        ...
        imageSrc: "https://postimg.cc/Vrb1tWPJ"
    }
};
...
function renderContent() {
    ...
    if (currentView === 'planner') {
        ...
        if (history.length === 0) {
            ...
            contentDiv.innerHTML = history.map((plan, index) => `
                <div class="mb-4 p-4 bg-gray-100 rounded-lg">
                    ...
                    <p class="mt-2">${lang.tasks_label}: ${plan.tasks.join(', ')}</p>
        ...
        contentDiv.innerHTML = `
            <div class="grid grid-cols-2 gap-2">
                ${Object.keys(themeIcons).map(style => `
                    <button class="btn secondary-btn" onclick="switchStyle('${style}')">${translations[currentLang][style]}</button>
                `).join('')}
            </div>
        `;
    } else if (currentView === 'theme') {
        contentDiv.innerHTML = `<button class="btn primary-btn" onclick="toggleTheme()">${lang.theme_title}</button>`;
    }
}
